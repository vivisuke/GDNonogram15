...
func _draw():
	...
	for x in range(N_TOTAL_CELL_HORZ+1):
		...
		draw_line(Vector2(x * CELL_WIDTH+1, y1), Vector2(x * CELL_WIDTH+1, y2), Color.black)
	var x2 = BOARD_WIDTH + 1
	for y in range(N_TOTAL_CELL_VERT+1):
		var x1 = 0 if y >= N_CLUES_CELL_HORZ || !y else CLUES_WIDTH
		draw_line(Vector2(x1, y * CELL_WIDTH+1), Vector2(x2, y * CELL_WIDTH+1), Color.black)
	...
	draw_line(Vector2(0, 0), Vector2(0, y2), Color.black)
	for x in range(4):
